...
from src.models import User, KBArticleStatus, VisibilityLevel
from src.schemas.kb import (
    ...
    KBArticleWorkflowUpdate, KBArticleVersionResponse, KBArticleListResponse
)
...
def list_categories(
    ...
    permission_engine.has_any_permission(
        db, None, current_user.id, current_user.role, current_user.organization_id,
...
def update_category(
    ...
    service = KBCategoryService(db)
    return service.update_category(category_id, data.model_dump(exclude_unset=True), current_user.organization_id)
...
def list_articles(
    ...
    items, total = service.search_articles(
        current_user.organization_id, query, category_id, actual_status, visibility, skip, size, sort_by
    )
...
def create_article(
    ...
    permission_engine.has_any_permission(
        db, None, current_user.id, current_user.role, current_user.organization_id,
        "create_kb_articles", "manage_knowledge_base"
    )
...
def update_article(
    ...
    permission_engine.has_any_permission(
        db, None, current_user.id, current_user.role, current_user.organization_id,
        "edit_kb_articles", "manage_knowledge_base"
    )
    
    service = KBArticleService(db)
    return service.update_article(
        article_id, data.model_dump(exclude_unset=True), current_user.organization_id, current_user.id, data.edit_reason
    )
...
def transition_article(
    ...
    permission_engine.has_any_permission(
        db, None, current_user.id, current_user.role, current_user.organization_id,
        "publish_kb_articles", "manage_knowledge_base"
    )
    
    service = KBArticleService(db)
    return service.transition_status(article_id, current_user.organization_id, data.status, current_user.id)
...
def delete_article(
    ...
    permission_engine.has_any_permission(
        db, None, current_user.id, current_user.role, current_user.organization_id,
        "delete_kb_articles", "manage_knowledge_base"
    )
...
def restore_article_version(
    ...
    service = KBArticleService(db)
    return service.restore_version(article_id, version, current_user.organization_id, current_user.id)
