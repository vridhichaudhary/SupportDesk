...
class TicketService(BaseService[Ticket, TicketCreate, TicketUpdate]):
    ...
    def create_ticket(
        ...
        try:
            ...
            structlog.get_logger().warning(
                "Routing task dispatch failed", error=str(e), ticket_id=str(ticket.id)
            )
    ...
    def reply(
        ...
        if not is_internal and ticket.status in [
            TicketStatus.NEW,
    ...
    def merge_tickets(
        ...
        merge_record = TicketMerge(
            source_ticket_id=source_id, target_ticket_id=target_id, merged_by_id=actor_id
        )
        db.add(merge_record)
