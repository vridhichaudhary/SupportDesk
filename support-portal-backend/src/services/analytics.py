...
class AnalyticsService:
    ...
    def get_executive_dashboard(
        self, db: Session, org_id: uuid.UUID, days: int = 30
    ) -> Dict[str, Any]:
        """
        ...
        total_tickets = (
            ...
            .filter(Ticket.organization_id == org_id, Ticket.created_at >= since_date)
            .scalar()
            or 0
        )
        ...
        sla_met = sum(
            1
            for t in total_resolved
            if t.sla_due_at and t.resolved_at and t.resolved_at <= t.sla_due_at
        )
        sla_compliance = (
            round((sla_met / len(total_resolved)) * 100, 1) if total_resolved else 100.0
        )
        ...
        ai_routed_tickets = (
            ...
            .filter(
                RoutingDecision.organization_id == org_id, RoutingDecision.created_at >= since_date
            )
            .scalar()
            or 0
        )
        ...
        ai_resolution_rate = (
            round((ai_routed_tickets / total_tickets) * 100, 1) if total_tickets > 0 else 0.0
        )
    ...
    def get_manager_dashboard(
        ...
        # Tickets by team
        tickets_by_team_raw = (
            db.query(Team.name, func.count(Ticket.id))
            .join(Ticket, and_(Ticket.assigned_team_id == Team.id))
            .filter(Ticket.organization_id == org_id, Ticket.created_at >= since_date)
            .group_by(Team.name)
            .all()
        )
    ...
    def get_agent_dashboard(
        self, db: Session, org_id: uuid.UUID, agent_id: uuid.UUID, days: int = 30
    ) -> Dict[str, Any]:
        """
...
class KPIEngine:
    ...
    def calculate_sla_compliance(tickets: List[Ticket]) -> float:
        if not tickets:
            return 100.0
        met = sum(
            1 for t in tickets if t.sla_due_at and t.resolved_at and t.resolved_at <= t.sla_due_at
        )
