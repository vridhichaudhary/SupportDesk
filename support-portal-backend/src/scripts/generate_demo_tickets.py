...
def generate_demo_data():
    ...
    try:
        ...
        if not customer:
            customer = Customer(
                organization_id=org.id, email="demo.customer@example.com", name="Demo Customer"
            )
            db.add(customer)
            db.commit()
            db.refresh(customer)
        ...
        agents = (
            db.query(User).filter(User.organization_id == org.id, User.role == UserRole.AGENT).all()
        )
        ...
        subjects = [
            ...
            "Billing issue with latest invoice",
        ...
        for _i in range(150):
            ...
            status = random.choice(
                [TicketStatus.RESOLVED, TicketStatus.RESOLVED, TicketStatus.OPEN, TicketStatus.NEW]
            )
