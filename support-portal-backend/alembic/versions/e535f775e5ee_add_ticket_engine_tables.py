...
def upgrade() -> None:
    # 1. Add new Enums
    sa.Enum('BILLING', 'TECHNICAL', 'BUG_REPORT', 'FEATURE_REQUEST', 'ACCOUNT', 'SECURITY', 'SALES', 'GENERAL', name='ticketcategory').create(op.get_bind())
    sa.Enum('EMAIL', 'WEB', 'API', 'INTERNAL', name='ticketsource').create(op.get_bind())
    sa.Enum('CUSTOMER_REPLY', 'AGENT_REPLY', 'INTERNAL_NOTE', 'SYSTEM_EVENT', name='threadtype').create(op.get_bind())
    ...
    op.add_column('customers', sa.Column('is_vip', sa.Boolean(), server_default='false', nullable=False))
    op.add_column('customers', sa.Column('total_tickets', sa.Integer(), server_default='0', nullable=False))
    ...
    op.add_column('tickets', sa.Column('created_by_id', postgresql.UUID(as_uuid=True), nullable=True))
    op.add_column('tickets', sa.Column('merged_into_id', postgresql.UUID(as_uuid=True), nullable=True))
    op.add_column('tickets', sa.Column('category', postgresql.ENUM('BILLING', 'TECHNICAL', 'BUG_REPORT', 'FEATURE_REQUEST', 'ACCOUNT', 'SECURITY', 'SALES', 'GENERAL', name='ticketcategory', create_type=False), server_default='GENERAL', nullable=False))
    op.add_column('tickets', sa.Column('source', postgresql.ENUM('EMAIL', 'WEB', 'API', 'INTERNAL', name='ticketsource', create_type=False), server_default='WEB', nullable=False))
    ...
    op.create_foreign_key('fk_ticket_department', 'tickets', 'departments', ['department_id'], ['id'], ondelete='SET NULL')
    op.create_foreign_key('fk_ticket_created_by', 'tickets', 'users', ['created_by_id'], ['id'], ondelete='SET NULL')
    op.create_foreign_key('fk_ticket_merged_into', 'tickets', 'tickets', ['merged_into_id'], ['id'], ondelete='SET NULL')
    
    op.execute("ALTER TABLE tickets ALTER COLUMN status SET DEFAULT 'NEW'")

    # 6. Create new tables
    op.create_table('ticket_threads',
        sa.Column('id', postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column('ticket_id', postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column('thread_type', postgresql.ENUM('CUSTOMER_REPLY', 'AGENT_REPLY', 'INTERNAL_NOTE', 'SYSTEM_EVENT', name='threadtype', create_type=False), nullable=False),
