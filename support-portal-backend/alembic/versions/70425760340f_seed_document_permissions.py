...
def downgrade() -> None:
    op.execute("DELETE FROM role_permissions WHERE permission_codename IN ('manage_documents', 'upload_documents', 'view_documents')")
    op.execute("DELETE FROM permissions WHERE codename IN ('manage_documents', 'upload_documents', 'view_documents')")
